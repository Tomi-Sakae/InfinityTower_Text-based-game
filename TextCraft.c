...
void thong_tin_nhan_vat()
{
    ...
    if (vat_pham[trang_bi].so_luong != 0 && vat_pham[trang_bi].gop == 0)
    {
...
int trang = 1;
int sl_vat_pham = 0; // cac bien toan cuc danh cho ham tui do
int kt_tui_do = 0;
void tui_do() // giao dien tui do
{
    ...
    int y_chon = 7;
    int y = 7;
    int vi_tri = sl_vat_pham;
    char lua_chon_menu;
    gotoxy(56, 5);
    printf("Tui do: ");
    for (i = sl_vat_pham; i < so_luong_vat_pham; i++) // hien thi cac vat pham ma nguoi choi co
    {
        gotoxy(58, y);
        puts(vat_pham[i].ten);
        if (vat_pham[i].so_luong != 0 && vat_pham[i].gop == 0)
        {
...
{
    int kt = 7; // kiem tra mau cua chu so
    gotoxy(56, 5);
    printf("Che tao: ");
    if (kiem_tra_vat_pham(1, 5) != 1) // neu nguoi choi khong co du vat pham thi hien chu do
        kt = 4;
    TextColor(kt);
    gotoxy(58, 7);
...
void di_chuyen_va_hanh_dong() // ham di chuyen vi tri va thuc hien hanh dong cua nguoi choi
{
    ...
    case 'e':
    {
        chuyen_chuc_nang();
        trang = 1;
        sl_vat_pham = 0; // reset lai cac bien dung cho tui do
        kt_tui_do = 0;
        tui_do();
        chuyen_chuc_nang();
        break;
...
void thuc_hien_hanh_dong()
{
    char so_thanh_chuoi[100];                                         // bien chuyen so thanh chuoi
    sprintf(so_thanh_chuoi, " %d ", vat_pham[trang_bi].suc_cong_pha); // lenh chuyen so thanh chuoi
    if (vat_the[i_tam].id == 1)
    {
        ...
        strcat(hanh_dong, so_thanh_chuoi); // hien thi suc cong pha cua trang bi hien tai
    ...
    if (vat_the[i_tam].id == 2)
    {
        ...
        {
            strcpy(hien_thi, "Ban can co cup de dao cuc da nay!");
            return;
        }
        the_luc -= 10;
...
void tao_moi_vat_pham(int id, int so_luong) // tao mot vat pham ma nguoi choi chua co
{
    if (id == 1)
    {
        ...
        vat_pham[so_luong_vat_pham].gop = 0; // 0 la vat pham co the gop lai
    ...
    if (id == 5)
    {
        ...
        vat_pham[so_luong_vat_pham].suc_cong_pha = 1;
        vat_pham[so_luong_vat_pham].gop = 1; // 1 la vat pham khong the gop duoc
        if (sua_loi_file == 0)
    ...
    if (id == 6)
    {
        ...
        if (so_luong_vat_pham == 0)
            ...
            vat_pham[so_luong_vat_pham].so_luong += so_luong;
        vat_pham[so_luong_vat_pham].the = 4; // 4 la vat pham dung de tac dong len cuc da
    }
    if (id == 7)
    {
...
void them_vat_pham(int id, int so_luong) // them vat pham vua nhan vao nguoi choi
{
    ...
    else
    {
        for (i = 0; i < so_luong_vat_pham; i++)
        {
            ...
            {
                if (vat_pham[i].gop == 1) // neu vat pham khong gop duoc thi tao vat pham moi
                    break;
